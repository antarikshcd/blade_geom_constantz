...
def bilinear_surface(surface_orig, Ncs_desired=500, Ns_desired=200):
      ...
      for j in range(Ns_desired):
        ...
        # X-value for the new surface
          surface_new[i, j, 0]= (float(Bx[0]) + float(Bx[1])*x_desired + 
                                 float(Bx[2])*y_desired + 
                                 float(Bx[3])*x_desired*y_desired)
        # Y-value for the new surface
          surface_new[i, j, 1]= (float(By[0]) + float(By[1])*x_desired + 
                                 float(By[2])*y_desired + 
                                 float(By[3])*x_desired*y_desired)
        
        # Y-value for the new surface
          surface_new[i, j, 2]= (float(Bz[0]) + float(Bz[1])*x_desired + 
                                 float(Bz[2])*y_desired + 
                                 float(Bz[3])*x_desired*y_desired)
   return surface_new          